...
<div class="userProfileCard clearFix<%=(UserInfo.ActivationStatus == EmployeeActivationStatus.Pending) ? " pending" : ""%>">
    <div class="additionInfo">
        <div class="profile-user-photo">
               ...
               { %>
                ...
                <div class="profile-status pending" data-visible="<%= UserInfo.Status == EmployeeStatus.Terminated ? "hidden" : "" %>">
                    ...
                    <div onclick="EmailOperationManager.ShowResendInviteWindow('<%= UserInfo.Email %>','<%= UserInfo.ID %>',<%= IsAdmin.ToString().ToLower() %>);return false;">
                        <%= Resource.PendingTitle %></div>
                    <% }
                       else if (UserInfo.ActivationStatus == EmployeeActivationStatus.NotActivated)
                       { %>
                    <div onclick="EmailOperationManager.ShowEmailActivationWindow('<%=UserInfo.Email%>','<%=UserInfo.ID%>',<%= IsAdmin.ToString().ToLower()%>);return false;">
                        <%= Resource.PendingTitle %></div>
                    <% } %>
                </div>
            ...
            title="<%= RoleIconName == "admin" ? Resource.Administrator : CustomNamingPeople.Substitute<Resource>("Guest").HtmlEncode() %>"></div>
            <%}%>
        </div>
                ...
                <tr>
                    <td>
                        ...
                        <div class="field">
                            ...
                            <span class="field-value"><%= CustomNamingPeople.Substitute<Resource>(UserInfo.IsVisitor() ? "Guest" : "User").HtmlEncode()%></span>
                        </div>
                        ...
                        <% if (!String.IsNullOrEmpty(UserInfo.Department)) { %>
                            <div class="field">
                                ...
                                <span class="field-value"><%=RenderDepartment()%></span>
                            </div>
                        <% } %>
                        <% if (!String.IsNullOrEmpty(UserInfo.Title)) { %>
                            <div class="field">
                                <span class="field-title describe-text"><%=CustomNamingPeople.Substitute<Resource>("UserPost").HtmlEncode()%>:</span>
                                <span class="field-value"><%=HttpUtility.HtmlEncode(UserInfo.Title)%></span>
                            </div>
                        <% } %>                       
                        <% if (Actions.AllowEdit && UserInfo.Status != EmployeeStatus.Terminated && UserInfo.ActivationStatus == EmployeeActivationStatus.Activated) { %>
                            <div class="field">
                                ...
                                <a onclick="javascript:AuthManager.ShowPwdReminderDialog('1','<%=UserInfo.Email%>'); return false;" class="baseLinkAction">&nbsp;</a>
                            </div>
                            <% if (UserInfo.IsMe())
                               { %>
                        ...
                        <% if(UserInfo.Sex.HasValue) { %>
                            <div class="field">
                                ...
                                <span class="field-value"><%=(UserInfo.Sex.HasValue ? UserInfo.Sex.Value ? Resource.MaleSexStatus : Resource.FemaleSexStatus : string.Empty)%></span>
                            </div>
                        <% } %>
                        <% if(UserInfo.WorkFromDate.HasValue) { %>
                            <div class="field">
                                ...
                                <span class="field-value"><%=UserInfo.WorkFromDate == null ? string.Empty : UserInfo.WorkFromDate.Value.ToShortDateString()%></span>
                            </div>
                        <% } %>
                        <% if(UserInfo.BirthDate.HasValue){ %>
                            <div class="field birthday">
                                <span class="field-title describe-text"><%=Resource.Birthdate%>:</span>
                                <span class="field-value"><%=UserInfo.BirthDate == null ? string.Empty : UserInfo.BirthDate.Value.ToShortDateString()%></span>
                        ...
                        <% if (!String.IsNullOrEmpty(UserInfo.Location)) { %>
                            <div class="field">
                                ...
                                <span class="field-value"><%=HttpUtility.HtmlEncode(UserInfo.Location)%></span>
                            </div>
                        <% } %>
                        <% if (AffiliateHelper.ButtonAvailable(UserInfo))
                           ...
                           <div class="field">
                                ...
                                <span class="field-value">
                                    ...
                                    <span class="HelpCenterSwitcher" onclick="jq(this).helper({ BlockHelperID: 'HelpJoinToAffilliate'});"></span><br />
...
<% if (!String.IsNullOrEmpty(UserProfile.Comment)) { %>
<div class="user-block profile-comment">
    ...
    <span id="commentContainer" class="inner-text tabs-content"><%= UserProfile.Comment.Replace("\n", "<br/>")%></span>
</div>
<% } %>

<% if (UserProfile.Phones.Count > 0 || UserProfile.Emails.Count > 0 || UserProfile.Messengers.Count > 0) { %>
<div class="user-block profile-contacts">
    ...
    <ul id="contactsPhoneContainer" class="contacts tabs-content">
        ...
        <% } %>
        <% if (UserProfile.Messengers.Count > 0) { %>
        <li class="contact">
...
<% } %>
<% if (UserProfile.Contacts.Count > 0) { %>
    ...
    <sc:Container runat="server" ID="_deleteProfileContainer">
        ...
        <Body>
            <div id="remove_content">
                ...
                <div class="link blue underline email"><%= UserInfo.Email %></div>
            </div>
            <div class="clearFix middle-button-container">
                <a href="javascript:void(0);" class="button blue middle" onclick="ProfileManager.SendInstrunctionsToRemoveProfile();"><%=Resource.SendButton%></a>
