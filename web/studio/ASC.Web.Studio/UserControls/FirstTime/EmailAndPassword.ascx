...
<div id="requiredStep" class="clearFix">
    ...
    <div class="portal">
        ...
        <% if (IsVisibleTestData)
           { %>
        <div class="test-data">
            <input type="checkbox" id="demo_data">
            <label for="demo_data"><%= Resources.Resource.TestDataText %></label>
            <span class="HelpCenterSwitcher" onclick="jq(this).helper({ BlockHelperID: 'TestDataHelp'});"></span>
        </div>
        <div class="popup_helper" id="TestDataHelp">
            <p><%= Resources.Resource.TestDataTextHelp %></p>
        </div>
        <% } %>
    </div>
</div>
