...
window.ASC.TMTalk.notifications  = (function () {
  var
    ...
    supported = {
      msie    : false,
      firefox : false,
      chrome  : typeof window.webkitNotifications === 'object',
      opera   : false,
      current : typeof window.webkitNotifications === 'object'
    };
  ...
  var init = function (iconpath, notifpath) {
    ...
    if (supported.current === true && window.webkitNotifications.checkPermission() === 0 && ASC.TMTalk.properties.item(propertyName) === '1') {
      isDisabled = false;
    }
  };
  ...
  var show = (function () {
    if (window.webkitNotifications) {
      return function (marker, title, content) {
        if (isDisabled) {
          return undefined;
        }
        ...
        //simple notification
        //try {
        //  var icon = handlerIconPath + '?id=' + marker;
        //  notification = window.webkitNotifications.checkPermission() === 0 ? window.webkitNotifications.createNotification(icon, title, content) : null;
        //} catch (err) {
        //  notification = null;
        //}
        ...
        //html notification
        try {
          if (handlerNotifPath !== null) {
            var uri = handlerNotifPath + '?t=' + encodeURIComponent(title) + '&c=' + encodeURIComponent(content) + '&i=' + encodeURIComponent(marker);
            notification = window.webkitNotifications.checkPermission() === 0 ? window.webkitNotifications.createHTMLNotification(uri) : null;
          }
        } catch (err) {
          notification = null;
        }
        if (notification !== null) {
          notification.onclick = getNotificationsClickCallback(marker);
          ...
          })(notification), nitificationTTL * 1000);
        }
      };
    }
    return function (marker, icon, title, content) {
      return undefined;
    };
  })();
    ...
    if (window.webkitNotifications) {
      return function (marker) {
        ...
        while (notificationsInd-- > 0) {
          if (notifications[notificationsInd].marker === marker) {
            notifications[notificationsInd].handler.cancel();
  ...
  var enable = (function () {
    if (window.webkitNotifications) {
      return function (callback) {
        if (window.webkitNotifications.checkPermission() === 1) { // 0 is PERMISSION_NOT_ALLOWED
...
//          if (chcPerm === null) {
//            chcPerm = {};
//            chcPerm.handler = setInterval((function (p) {
//              return function () {
//                if (window.webkitNotifications.checkPermission() !== 1) {
