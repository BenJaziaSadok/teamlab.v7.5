...
<script id="messageTmpl" type="text/x-jquery-tmpl">
    <div class="itemWrapper">
        <div class="head-subject">
            ...
            <div class="importance pull-left"><i class="icon-{{if $item.important==false}}un{{/if}}important"></i></div>
            <div class="viewTitle">
                {{if typeof($item.oldest_message.subject)==='undefined' || $item.oldest_message.subject == null || $item.oldest_message.subject ==''}}
                    <%: MailResource.NoSubject %>
                {{else}}
                    {{html $item.wordWrap($item.oldest_message.subject) }}
                {{/if}}
                ...
                <div class="row" data_id="${id}">
                    ...
                    <div class="value">
                        ...
                        {{if crm_available && isFromCRM != true && folder == 1 && from != 'mail-daemon@teamlab.com'}}
                            <div class="AddToCRMContacts">
                                <i class="icon-crm-plus"></i>
                                <span><%: MailResource.AddToCRMContacts %></span>
                            ...
                            { %>
                                {{if attachments.length > 1}}
                                    <div class="attachments-buttons">
                                        ...
                                        <a href="${download_all_url}" target="_blank" class="baseLinkAction"><%: MailResource.AttachDownloadAll %></a>
                                    </div>
                                {{/if}}
                        <% } %>
                    {{/if}}
...
<script id="messageBlockContent" type="text/x-jquery-tmpl">
    <div id="id_block_content_popup_${id}" class="yellow_help_wrap hidden">
        <div class="yellow_help">
            <span class="text"><%: MailResource.BlockedContentWarning %></span>
            <a id="id-btn-block-content-${id}" class="baseLinkAction" href="#" onclick="return false;"><%: MailResource.DisplayImagesLabel %></a>
            <a id="id-btn-always-block-content-${id}" class="baseLinkAction" href="#" onclick="return false;" style="margin-left: 8px;"><%: MailResource.AlwaysDisplayImagesLabel %> "${sender_address}"</a>
            <a class="close_yellow_help" href="#" onclick="jq('#id_block_content_popup_${id}').hide(); return false;"></a>
        </div>
    </div>
</script>
