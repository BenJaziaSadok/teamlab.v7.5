...
<% if (CanCreate || CurrentPage == "tmdocs")
   {%>
...
<%if (CanCreate || CurrentPage == "tmdocs")
  {%>
    <ul class="menu-actions">
        <li id="menuCreateNewButton" class="menu-main-button without-separator <%= CurrentPage == "tmdocs" ? "middle" : "big" %>">
            <span class="main-button-text" style="<%= CurrentPage == "tmdocs" ? "padding-top:8px;" : "" %>"><%= ProjectsCommonResource.CreateNewButton %></span>
...
<%}%>
        <ul class="menu-list">
            <li id="menuProjects" class="menu-item sub-list<%if(CurrentPage=="projects" && !IsInConcreteProject()){%> active currentCategory <% } if((CurrentPage=="projects" && !RequestContext.IsInConcreteProject) || IsInConcreteProject()){%> currentCategory<%} %>">
                ...
                <ul class="menu-sub-list">
                    <% if (MyProjects.Count != 0){%>
                       <li id="myProjectsConteiner" class="menu-sub-item<%if(IsInConcreteProject()){%> currentCategory<%}%>">
                            ...
                            <ul class="menu-sub-list">
                                <% foreach (var myProject in MyProjects)
                                   {%>
                                    <li class="menu-sub-item <%if(Project!= null && myProject.ID == Project.ID){%> active currentCategory <%}%>"><a href="tasks.aspx?prjID=<%= myProject.ID %>" title="<%= HttpUtility.HtmlEncode(myProject.Title)%>"><%= HttpUtility.HtmlEncode(myProject.Title)%></a></li>
                                   <% } %>
                            </ul>
                        </li>
                    <% }else{%>
            ...
            <li id="menuMilestones" class="menu-item sub-list <%if(CurrentPage=="milestones"){%>active currentCategory<%}%>">
                <div class="category-wrapper">
                    <span class="expander"></span>
            ...
            <li id="menuTasks" class="menu-item <%if (!Page.Participant.IsVisitor) {%>sub-list <%}else {%> none-sub-list <%} if(CurrentPage=="tasks"){%>active currentCategory<%}%>">
                <div class="category-wrapper">
                    <span class="expander"></span>
            ...
            <li id="menuMessages" class="menu-item sub-list <%if(CurrentPage=="messages"){%>active currentCategory <%}%>">
                <div class="category-wrapper">
                    <span class="expander"></span>
            ...
            <%if (ParticipantSecurityInfo["Time"])
              {%>
            <li id="menuTimeTracking" class="menu-item none-sub-list <%if(CurrentPage=="timeTracking"){%>active currentCategory<%}%>">
            ...
            <li class="menu-item
                <% if(CurrentPage == "tmdocs"){ 
                    %> sub-list open <% if (!RequestContext.IsInConcreteProject){%>active currentCategory<% }
                }else{
                   %>none-sub-list<% 
                }%>">
                <% if (CurrentPage == "tmdocs"){%>
                <div class="category-wrapper">
                    <span class="expander"></span>
                    <a class="menu-item-label outer-text text-overflow" href="<%= (CurrentPage == "tmdocs" && string.IsNullOrEmpty(Request[UrlConstant.ProjectID])) ? "#" : "tmdocs.aspx" %>">
                        <span class="menu-item-icon documents"></span>
                        <span class="menu-item-label inner-text"><%= ProjectsFileResource.Documents %></span>
                    </a>
                    <span class="is-new" title="<%= ASC.Web.Files.Resources.FilesUCResource.RemoveIsNew %>" data-id="<%= ASC.Web.Files.Classes.Global.FolderProjects %>"></span>
                </div>
            ...
            <% if (!Page.Participant.IsVisitor)
               { %>
                <li class="menu-item none-sub-list <% if (CurrentPage == "reports"){ %>active<% } %>">
                ...
                <% if (IsFullAdmin || IsProjectAdmin)
                   { %>
                    <li id="menuSettings" class="menu-item sub-list add-block <% if (CurrentPage == "projectTemplates" || CurrentPage == "import")
                                                                                 { %> currentCategory<% } %>">
                        <div class="category-wrapper">
                            <span class="expander"></span>
                        ...
                        <ul class="menu-sub-list">
                            <% if (IsFullAdmin || IsProjectAdmin)
                               { %>
                                <li class="menu-sub-item filter <% if (CurrentPage == "projectTemplates"){ %>active <% } %>">
                            ...
                            <% if (IsFullAdmin)
                               { %>
                                <li class="menu-sub-item filter <% if (CurrentPage == "import"){ %>active currentCategory<% } %>">
                                    <a id="menuImport" class="menu-item-label outer-text text-overflow" href="import.aspx"><%= ImportResource.Import %></a>
                                </li>                   
                              <li class="menu-sub-item filter">
                                    <a id="menuAccessRights" class="menu-item-label outer-text text-overflow" href="<%= VirtualPathUtility.ToAbsolute("~/management.aspx") + "?type=8#projects" %>"><%= ProjectResource.AccessRightSettings %></a>
                              </li>  
                            <% } %>                          
                        </ul>
                    </li>
...
<div class="mobileApp-banner display-none">
    ...
    <div class="mobileApp-banner_btns">
        ...
        <a href="#" target="_blank" class="mobileApp-banner_btn google-play"></a>
    </div>
</div>
<%} %>
