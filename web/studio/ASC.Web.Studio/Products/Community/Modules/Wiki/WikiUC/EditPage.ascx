...
<script language="javascript" type="text/javascript">
    var WikiPlugName = '<%=PathFromFCKEditor%>' + 'fck_wysiwyg';
    var WikiMobile = '<%=ASC.Web.Core.Mobile.MobileDetector.IsRequestMatchesMobile(HttpContext.Current)%>';
    ...
    function <%=this.ClientID%>_BlockButtons()
    {
        jq('#' + panelEditBtnsID).hide();
        jq('#action_loader').show();
    }
    
    function <%=this.ClientID%>_UnBlockButtons()
    {
        jq('#action_loader').hide();
        jq('#' + panelEditBtnsID).show();
    }
    ...
    function <%=this.ClientID%>_ShowPreview()
    {
        ...
        {
            if(b){<%=this.ClientID%>_BlockButtons();}
            else{<%=this.ClientID%>_UnBlockButtons();}
        };
