...
namespace ActiveUp.Net.Mail
{
    ...
    public abstract class Codec
    {
        ...
        /// <returns>conmbined encoded string</returns>
        private static string CombineSameEncodedParts(string input, Dictionary<string, List<byte>> decoded_bytes)
        {
            if (input.Contains("?==?"))
            {
                ...
                if (matches.Count > 1)
                {
                    ...
                    for (int i = 1; i < matches.Count; i++)
                    {
                        ...
                        {
                            if (input[matches[i].Index - 1] == '=' && input[matches[i].Index - 2] == '?')
                            {
                                ...
                                {
                                    var tmpArray = String2DecodedArray(last_message.Value, last_encoding);
                                    
                                    if (decoded_bytes.ContainsKey(last_content_type))
                                    {
                                        decoded_bytes[last_content_type].AddRange(tmpArray);
                                    }
                                    else
                                        decoded_bytes.Add(last_content_type, new List<byte>(tmpArray));
                                }
                                ...
                                return CombineSameEncodedParts(input, decoded_bytes);
                            }
                        }
                        else
                        {
