...
{
    public class InMemoryTokenManager : IAssociatedTokenManager, IOpenIdOAuthTokenManager
    {
        private readonly Dictionary<string, string> tokensAndSecrets = new Dictionary<string, string>();
        private readonly Dictionary<string, string> tokensAndAssociations = new Dictionary<string, string>();
        ...
        {
            return tokensAndSecrets.Keys;
        }

        public IEnumerable<string> GetAssociations()
        {
            return tokensAndAssociations.Values;
        }

        public string GetAssociation(string token)
        {
            return tokensAndAssociations[token];
        }

        public IEnumerable<string> GetTokensByAssociation(string associateData)
        {
            return tokensAndAssociations.Where(x => x.Value == associateData).Select(x => x.Key);
        }

        public void AssociateToken(string token, string associateData)
        {
            tokensAndAssociations.Add(token,associateData);
        }

        public void RemoveAssociationFromToken(string token)
        {
            tokensAndAssociations.Remove(token);
        }

        public void ExpireToken(string token)
        {
            tokensAndSecrets.Remove(token);
        }
    }
}