...
namespace ASC.Mail.Net.SIP.Stack
{
    ...
    public class SIP_RequestSender : IDisposable
    {
        ...
        private void ClientTransaction_TimedOut(object sender, EventArgs e)
        {
            lock (m_pLock)
            {
                ...
                */
                ...
                {
                    OnResponseReceived(m_pStack.CreateResponse(SIP_ResponseCodes.x408_Request_Timeout,
                                                               m_pRequest));
                    OnCompleted();
                }
            }
                ...
                else
                {
                    OnResponseReceived(
                        m_pStack.CreateResponse(
                            SIP_ResponseCodes.x503_Service_Unavailable + ": Transport error.", m_pRequest));
                    OnCompleted();
                }
            }
        }
        ...
        private void SendToNextHop()
        {
            ...
            SendToFlow(m_pStack.TransportLayer.GetOrCreateFlow(hop.Transport, null, hop.EndPoint),
                       m_pRequest.Copy());
        }
        ...
        #region Nested type: SIP_RequestSenderState
