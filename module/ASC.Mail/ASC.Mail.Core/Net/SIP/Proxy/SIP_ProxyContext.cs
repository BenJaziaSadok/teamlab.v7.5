...
namespace ASC.Mail.Net.SIP.Proxy
{
    ...
    {
        #region Nested type: TargetHandler
        ...
        private class TargetHandler : IDisposable
        {
            ...
            public void Start()
            {
                lock (m_pLock)
                {
                    ...
                    if (m_pHops.Count == 0)
                    {
                        m_pOwner.ProcessResponse(this,
                                                 m_pTransaction,
                                                 m_pOwner.Proxy.Stack.CreateResponse(
                                                     SIP_ResponseCodes.x503_Service_Unavailable +
                                                     ": No hop(s) for target.",
                                                     m_pTransaction.Request));
                        Dispose();
                        return;
                    }
                    else
                    ...
                    */
                    else
                    {
                        ...
                        m_pOwner.ProcessResponse(this,
                                                 m_pTransaction,
                                                 m_pOwner.Proxy.Stack.CreateResponse(
                                                     SIP_ResponseCodes.x408_Request_Timeout,
                                                     m_pTransaction.Request));
                        Dispose();
                    }
                }
            ...
            private void SendToNextHop()
            {
                ...
                SendToFlow(
                    m_pOwner.Proxy.Stack.TransportLayer.GetOrCreateFlow(hop.Transport, null, hop.EndPoint),
                    m_pRequest.Copy());
            }
        ...
        private bool m_IsStarted;
        private SIP_ProxyCore m_pProxy;
